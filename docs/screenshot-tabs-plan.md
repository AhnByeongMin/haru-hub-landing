...
2. **데모 계정 + 더미 데이터** — 미착수.
   - lease: ✅ **캡처 3장 완료**(목록/상세/캘린더, `assets/screenshots/lease-*.png`). 데모 방법 = id1 name 임시 '데모 임대인' + **'우리집'(실주소) HIDDEN 숨김** + 상세는 가짜매물 id5(망원동). 캡처 후 id1 name 원복(안병민)·우리집은 HIDDEN 유지(실주소라 안전, id6 원래값 미백업).
     ⚠️ 캡처 검증 중 발견: 첫 매물 '우리집'이 **실주소 + 엑셀 사진**이라 못 씀 → 가짜매물(망원동, 사진없어 색 placeholder)로 교체. **lease-4 공지(`announce`)는 내용 'ㅇㅇ' 빈약 → 보강 후 재캡처 필요.**
